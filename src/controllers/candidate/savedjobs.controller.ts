...
export const manageSavedJobs = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  ...
  try {
    ...
    const existingSavedJobs = await SavedJobs.findOne({ userId },{_id:1}).session(session);
    ...
    return res.status(200).json({ message: `${operation === 'add' ? 'Added' : 'Removed'}` ,success: true,});
  } catch (error) {
...
export const getSavedJobsAll = async (req: Request, res: Response,next:NextFunction) => {
    try {
      const userId = res.locals.userId as Types.ObjectId
